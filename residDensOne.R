...
for (i in 1:4) {
  ...
  resid <- apply(total.data[[i]], 1, Minus)
  plot(density(resid), main = paste('Total Time Point', i))
...
for (i in 1:4) {
  ...
  y <- ribo.data[[i]][,2]
  fit <- lm(log10(y) ~ log10(x))
  resid <- apply(ribo.data[[i]], 1, Minus)
  plot(density(resid), main = paste('Ribosomal Time Point', i))
#  plot(x, y, log = 'xy', xlab = '', ylab = '', xlim = xlim, ylim = ylim,
